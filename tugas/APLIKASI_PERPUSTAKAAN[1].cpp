...
void cariBuku() {
    ...
    bool ditemukan = false; // Variabel untuk mengecek apakah buku ditemukan

    for (int i = 0; i < jumlahBuku; i++) {
        if (perpustakaan[i].judul.find(judul_buku) != string::npos) { // Mencari judul yang cocok
            ...
            cout << "ID Buku   : " << perpustakaan[i].id_buku << endl;
            cout << "Jumlah Stok: " << perpustakaan[i].jumlah_stok << endl;
            cout << "-----------------------------" << endl;
            ditemukan = true; // Set menjadi true jika buku ditemukan
        }
    }
...
int main() {
    ...
    do {
        ...
        cout << "6. Keluar" << endl;
        cout << "Masukkan pilihan yang diinginkan: ";
        cin >> pilihan;
